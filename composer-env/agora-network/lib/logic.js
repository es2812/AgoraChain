...
async function addRestrictionTransaction(tx) {
    ...
    if(valid){
        const factory = await getFactory();
        //Not all restrictions are available to participant, so client API can't
        //just generate a unique ID via queries.
        //
        //We guarantee uniqueness by using restrictionID=hash(client-given ID+trusteeID)
        //so that the client API can just generate client-given ID by checking the rest
        //of Restriction assets from the trustee, which should be available.
        let id = Sha256.hash(tx.restrictionID+tx.trustee.getIdentifier())

        const restrictionRegistry = await getAssetRegistry(NS+'.Restriction');
        let restriction = factory.newResource(NS,'Restriction',id);
...
async function createElectionTransacton(tx){
    ...
    //Client API should query existing Election assets to get a unique ID
...
async function publicVoteTransaction(tx){
    ...
    if(tx.election.closed){
        ...
        //we use concat(electionID,voterID) as the id for the vote
...
async function secretVoteTransaction(tx){
    ...
    if(tx.envelope.election.closed){
        ...
        if(isFirstVote){
            ...
            //for the voteID we can use a hash of the envelopeID, since that will be unique
            //hashing the id will make it more difficult to make the connection between vote->envelope,
            //even though envelopes are private
            let id = Sha256.hash(tx.envelope.getIdentifier())
...
async function envelopeTransaction(tx){
    ...
    if(envelopes.length==0){
        ...
        if(tx.election.closed){
            ...
            //Client API can't query all of the envelopes for a unique value for the id
            //but it can query the voter's envelopes, so it only needs to get a unique value
            //among them and the function will assign id=hash(client-given ID+voterID)
            let factory = await getFactory();
            let id = Sha256.hash(tx.envelopeID+tx.voter.getIdentifier())
            let envelope =  factory.newResource(NS,'Envelope',id);
        ...
        throw new Error(tx.voter+" already has an envelope for "+tx.election);
    }    
}
